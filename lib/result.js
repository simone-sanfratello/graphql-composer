...
function mergeResult (result, fullPath, queryNode, parentResult) {
  ...
  if (Array.isArray(r)) {
    for (let i = 0; i < r.length; i++) {
      copyResultRow(r[i], partial, index, parentKey, parentResult.path)
    }
    return
  }

  // r is an object
  copyResultRow(r, partial, index, parentKey, parentResult.path)
}

function copyResultRow (dst, src, srcIndex, parentKey, path) {
  // TODO if srcIndex.list > copy resultS

  if (Array.isArray(dst)) {
    for (let i = 0; i < dst.length; i++) {
      const row = dst[i]
      copyResultRow(row, src, srcIndex, parentKey)
    }
    return
  }
  ...
  if (!dst?.[parentKey]) {
    // TODO will be used for "as" entities option
    return
  }
