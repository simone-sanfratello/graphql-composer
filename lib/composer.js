...
const { unwrapFieldTypeName, objectDeepClone, pathJoin } = require('./utils')
...
class Composer {
      ...
      for (let i = 0; i < query.query.selection.length; i++) {
        ...
        if (s.deferreds) {
          for (const deferred of s.deferreds.values()) {
            // deferred.queryNode.field.typeName is entity typeName
            // TODO group keys by path/entity, this will simplify "parentKey" fn later
            // for example: artists.songs.singer.lastName and artists.songs.singer.firstName > artists.songs.singer
            // use deferred.resolverPath
            if (!deferred.keys) {
              deferred.keys = entityKeys({ field: deferred.queryNode.field, entity: deferred.queryNode.field.typeName })
            } else {
              // TODO log, warn
              console.error('defered already have keys!')
            }
          }
        }
      }
  ...
  buildDeferredQueries (deferred, context) {
    ...
    for (const { fieldName, queryFieldNode } of deferred.fields) {
      ...
      if (!subgraphs[subgraphName]) {
        const entity = this.getEntity(deferred.queryNode.field.typeName, subgraphName)
        if (!entity) {
          console.error(' ---------- ERROR no entity', deferred.queryNode.field.typeName, '@', subgraphName)
  ...
  getParentResult (queryNode, path) {
    ...
    if (q.keys.length > 0) {
      ...
      if (queryParentKey) {
        result.path = pathJoin(queryParentKey.path, queryParentKey.key).split('.')// .slice(1)
        console.log('\n\n\n----')
        console.log('queryParentKey.path', queryParentKey.path)
        console.log('queryParentKey.key', queryParentKey.key)
...
function parentKey (keys, path, type) {
  ...
  if (fkeys.length > 0) {
    // && k.fkey.type === type)  {
    // k.fkey.type === type ? k.fkey.field : k.fkey.pkey
    // console.log(' ************** TODO !!!')
    const k = fkeys[0]
...
function mergeQueries (currentQuery, query) {
  // no worries, will be filtered later on building
