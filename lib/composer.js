...
class Composer {
  ...
  collectQueries ({
    ...
    for (const queryFieldNode of info.fieldNodes) {
      ...
      this.buildQueries(q, queries, context)
    }
  ...
  buildQueries (collectedQueries, queries, context) {
      ...
      for (let i = 0; i < query.query.selection.length; i++) {
        ...
        if (s.deferreds) {
          for (const deferred of s.deferreds.values()) {
            deferred.keys = entityKeys(deferred.queryNode.field) // ? query.subgraphName)
          }
        }
      }
    ...
    for (const deferred of collectedQueries.deferreds.values()) {
      ...
      for (const deferredQueries of deferredQueriesBatch) {
        this.buildQueries(deferredQueries, queries, context)
      }
    }
  }
  ...
  buildDeferredQueries (deferred, context) {
    ...
    for (const { fieldName, queryFieldNode } of deferred.fields) {
      ...
      if (!subgraphs[subgraphName]) {
        // const entity = deferred.queryNode.parent
        //   ? this.getEntity(deferred.queryNode.parent.field.typeName, subgraphName)
        //   : this.getEntity(deferred.queryNode.field.typeName, subgraphName)
    ...
    for (const subgraphName of Object.keys(subgraphs)) {
      ...
      // console.log('deferred query:', deferred.queryNode.path, 'parent', deferred.queryNode.field.name, '@', deferred.queryNode.subgraphName)
  ...
  getParentResult (queryNode) {
    ...
    if (q.keys.length > 0) {
      const key = parentKey(q.keys, queryNode.field.typeName)
      if (key) {
