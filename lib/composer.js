...
class Composer {
  ...
  buildDeferredQueries ({ fieldName, queryFieldNode, parent, path }, info) {
    ...
    const subgraphName = this.deferredSubgraph(parent.field.typeName, fieldName)
  ...
  async runQueries (queries) {
    ...
    for (const [path, q] of queries) {
      ...
      console.log({ subgraph: q.subgraphName, query })
  ...
  getParentResult (queryNode) {
    ...
    if (q.keys.length > 0) {
      const k = q.keys.find(k => k.type === queryNode.field.parent.name)
      if (k) {
        result.path = k.pkey.split('.')
