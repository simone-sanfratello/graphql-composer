...
class Composer {
  ...
  deferredSubgraph (fieldId) {
    ...
    const subgraphName = Object.keys(field)[0]
    return subgraphName
  }
  ...
  getAlias (fieldId, subgraphName) {
    return this.aliases[fieldId]?.[subgraphName]
  }
  ...
  buildDeferredQueries (deferred, context) {
    ...
    for (const { fieldName } of deferred.fields) {
      ...
      if (alias) {
        ...
        subgraphs[alias.subgraph].fields.push({ fieldName })
      } else {
...
function parentKey (keys, path, type) {
  ...
  for (const k of keys) {
    ...
    if (k.many) {
      console.log(' ************** TODO parentKey many')
      // many.push(k)
    }
  }
