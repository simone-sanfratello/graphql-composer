...
class Composer {
  ...
  collectQueries ({
    ...
    for (const queryFieldNode of info.fieldNodes) {
      ...
      this.buildQueries(q, queries)
    }

    return queries
  }

  buildQueries (collectQueries, queries) {
    for (const [path, query] of collectQueries.queries) {
      if (queries.has(path)) {
    ...
    for (const [path, deferred] of collectQueries.deferreds) {
      if (queries.has(path)) {
      ...
      // recursively
      // result queries should be already partitioned by subgraph?

      const deferredQueries = this.buildDeferredQueries(deferred)
      this.buildQueries(deferredQueries, queries)
    }
  }

  // build query from deferred info
  buildDeferredQueries ({ fieldName, parent }) {
    const { subgraphName, field, query } = this.computeDeferredQuery({ fieldName, parent })
  ...
  computeDeferredQuery ({ fieldName, parent }) {
    // this.logger.debug({ fieldName, parent }, 'composer.computeDeferredQuery')
    ...
    // TODO a deferred query may generate multiple queries
    const query = this.computeQuery({ field, subgraphName, parent })
  ...
  computeQuery ({ field, subgraphName, parent }) {
    ...
    const s = [{ field: entity.pkey }, { field: field.name }]

    // collect queries on this subgraph
    // collectQueries()
