...
class Composer {
  ...
  buildDeferredQueries ({ fieldName, queryFieldNode, parent, path }, info) {
    ...
    const q = collectQueries({
      ...
      // TODO sync with buildQueryEntityKeys to pick up keys
      selection: [{ field: entity.pkey }]
    })
  ...
  async runQueries (queries) {
    ...
    for (const [path, q] of queries) {
      // TODO move to own fn "collectKeys"
      const queryParent = queryParentResult(q.parent)
      let resultPath = []
      let parentResult
      let parentKey = q.parent?.field.type.entity?.pkey

      if (queryParent) {
        parentResult = queryParent.result

        if (queryParent.keys.length > 0) {
          // TODO fkeys, many, pkey ...
          parentKey = queryParent.keys.find(k => k.type === q.field.parent.name)
          if (parentKey) {
            parentKey = parentKey.pkey
            resultPath = parentKey.split('.')
          }
        }
      }
      // --- fn

      const { query, fieldName, keys } = buildQuery(q.query, q.subgraphName, parentResult, resultPath)
      ...
      mergeResult(result, path, parentKey, q)
    }
