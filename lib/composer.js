...
class Composer {
  ...
  async setupComposerSubgraph () {
    ...
    for (const name of Object.keys(entities)) {
      ...
      const subgraph = this.subgraphs.get(entity.subgraphName)
      // TODO remove this if when entities options will be validated
      if (subgraph) {
  ...
  resolveEntities () {
    ...
    const entitiesSubgraphs = Object.keys(this.entities)
    if (entitiesSubgraphs.length < 1) {
      return { schema: undefined, resolvers: undefined, entities: undefined }
    }

    for (const subgraphName of entitiesSubgraphs) {
      const entities = this.entities[subgraphName]
      const entityNames = Object.keys(entities)

      // TODO handle multiple/differnt keys in different subgraphs for an entity

      for (const entityName of entityNames) {
        topEntities.set(entityName, {
          subgraphName,
          pkey: entities[entityName].pkey,
          fkeys: new Map()
        })
      }
      ...
      for (const entityName of entityNames) {
        ...
        // pkey
        const type = schemaTypeName(this.types, subgraphName, entityName, entity.pkey)
        entitySchemaFields[entity.pkey] = type

        // fkeys
        if (entity.fkeys) {
          for (const fkey of entity.fkeys) {
            setEntityFKey(topEntities.get(entityName).fkeys, fkey)
            entitySchemaFields[fkey.as] = fkey.type

            // TODO? resolver will be replaced on query building
          }
        }
