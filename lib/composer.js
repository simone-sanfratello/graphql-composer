...
class Composer {
  ...
  deferredSubgraph (fieldId) {
    ...
    if (!field) {
      ...
      console.log(' ------- ERROR no field', fieldId)
    }
        ...
        if (s.deferreds) {
          for (const deferred of s.deferreds.values()) {
            ...
            if (!deferred.keys) {
              ...
              console.log('defered already have keys!')
            }
          }
        }
      }
  ...
  buildDeferredQueries (deferred, context) {
    ...
    for (const { fieldName, queryFieldNode } of deferred.fields) {
      ...
      if (!subgraphs[subgraphName]) {
        ...
        if (!entity) {
          console.log(' ---------- ERROR no entity', deferred.queryNode.field.typeName, '@', subgraphName)
  ...
  getParentResult (queryNode, path) {
    ...
    if (q.keys.length > 0) {
      ...
      if (queryParentKey) {
        // TODO slice(1) may not be 100% correct
...
function parentKey (keys, path, type) {
  ...
  if (fkeys.length > 0) {
    ...
    console.log(' ************** TODO !!!')
    const k = fkeys[0]
    return { key: k.fkey.field ?? k.pkey, path: k.resolverPath }
  }
