...
class Composer {
  ...
  // set this.fieldsSubgraph[fieldId] on merge schema to avoid this logic and just return this.fieldsSubgraph[fieldId]
  deferredSubgraph (fieldId) {
    const field = this.fields[fieldId]
    if (!field) {
      // TODO log err, throw err
      console.error(' ------- deferredSubgraph ERROR no field', fieldId)
      return
    }
  ...
  async setupComposerSubgraph () {
    ...
    for (const fieldId of Object.keys(this.aliases)) {
      ...
      // ??? const f = this.addField(alias.parentType, field, alias.entity.subgraph, t, alias.resolver)
  ...
  buildQueries (collectedQueries, queries, order, context) {
    for (const deferred of collectedQueries.deferreds.values()) {
      ...
      if (!deferred.queryNode.field) {
        console.log(' >>> DEFERRED FIELD ', deferred.queryNode.fieldId)
        // deferred.queryNode.field = Object.values(this.fields[deferred.queryNode.fieldId])[0]
        deferred.queryNode.field = deferred.queryNode.parent.field
        // TODO if no field throw new Error('NO_DEFERRED_FIELD')
      }
  ...
  buildDeferredQueries (deferred, context) {
    ...
    for (const { fieldName, alias } of deferred.fields) {
      ...
      // TODO cleanup code
      // const a = this.aliases[fieldId]?.[subgraphName]
      // const a = alias?.[subgraphName]
      const a = alias && Object.values(alias)[0].subgraph === subgraphName ? alias : null

      if (a) {
        console.log(' AAAAAAAAAAAAAAAAAa ALIAS ', fieldId)
        this.addDeferredSubgraph(subgraphs, a.subgraph, a.type, deferred.keys, deferred.queryNode)
        subgraphs[a.subgraph].fields.push({ fieldName })
      } else {
...
function setEntityMany (manys, many) {
  const index = `${many.type}.${many.field}`
