...
class Composer {
  ...
  deferredSubgraph(fieldId) {
    ...
    if (!field) {
      // TODO log err, throw err
      console.error(' ------- ERROR no field', fieldId)
    }
    // TODO handle edge-case: can the field be resolved by multiple subgraphs?
  ...
  async setupComposerSubgraph() {
    ...
    for (const name of Object.keys(entities)) {
      ...
      const subgraph = this.subgraphs.get(entity.subgraph)
      subgraph.entities.set(name, entity)
      this.subgraphs.set(entity.subgraph, subgraph)
    }
  ...
  mergeSchema({ __schema: schema, subgraphName }) {
    ...
    // implement here alias for subgraphs
    // TODO move to own fn
    if (this.entities && subgraphName === COMPOSER_SUBGRAPH_NAME) {
      for (const entityName of Object.keys(this.entities)) {
        const entity = this.entities[entityName]

        // !!!! TODO CHECK need for deferred.keys
        // fill entities in composer
        this.setEntity(entityName, entity.subgraph, entity)
        const type = this.types[entityName][entity.subgraph].src
        this.addType(entityName, entity.subgraph, type, entity, true)        
        // !!!! TODO CHECK

        if (entity.fkeys) {
          for (const fkey of entity.fkeys) {
            if (!fkey.as) { continue }
            this.setAlias(`${entityName}.${fkey.as}`, subgraphName, { ...fkey, mode: 'fkey' })
          }
        }
        ...
        if (entity.many) {
          for (const many of entity.many) {
            if (!many.as) { continue }
            this.setAlias(`${entityName}.${many.as}`, subgraphName, { ...many, mode: 'many' })
          }
        }
      }
    }
    ...
    for (const fieldId of Object.keys(this.fields)) {
      for (const subgraphName of Object.keys(this.fields[fieldId])) {
        ...
        const alias = this.aliases.get(fieldId)
        if (!alias) { continue }
        f.alias = alias
      }
    }
  ...
  addType(typeName, subgraphName, type, entity, override = false) {
    if (!override && this.types[typeName] && this.types[typeName][subgraphName]) {
  ...
  setAlias(fieldId, subgraphName, alias) {
    this.aliases.set(`${fieldId}#${subgraphName}`, alias)
  }

  getAlias(fieldId, subgraphName) {
    return this.aliases.get(`${fieldId}#${subgraphName}`)
  }
  ...
  async runResolver({ typeName, subgraphName, fieldSrc, parent, args, context, info }) {
    ...
    const result = await this.runQueries(queries, order)
  ...
  buildQueries(collectedQueries, queries, order, context) {
    ...
    for (let [path, query] of collectedQueries.queries) {
      ...
      query.query.selection.push(...entityKeys({ field: query.field }).map(key => ({ key })))
  ...
  buildDeferredQueries(deferred, context) {
    ...
    for (const { fieldName, queryFieldNode } of deferred.fields) {
      let fieldId = createFieldId(deferred.queryNode.field.typeName, fieldName)
      let subgraphName = this.deferredSubgraph(fieldId)
      let entity, resolver

      const alias = this.getAlias(fieldId, subgraphName)
      if (alias) {
        console.log(alias)
        fieldId = createFieldId(deferred.queryNode.field.typeName, alias.field)
        const aliasSubgraphName = this.deferredSubgraph(fieldId)
        entity = this.getEntity(deferred.queryNode.field.typeName, subgraphName)
        resolver = deferredResolver(entity, deferred.keys, aliasSubgraphName)
        subgraphName = aliasSubgraphName
      }

      if (!subgraphs[subgraphName]) {
        if (!entity) {
          entity = this.getEntity(deferred.queryNode.field.typeName, subgraphName)
        }

        if (!entity) {
          this.logger.error({ entityName: deferred.queryNode.field.typeName, subgraphName }, 'missing entity, unable to compute deferred query')
          throw new Error('UNABLE_BUILD_DEFERRED_QUERY_MISSING_ENTITY')
        }

        if (!resolver) {
          resolver = deferredResolver(entity, deferred.keys, subgraphName)
        }
        subgraphs[subgraphName] = { fields: [], resolver, entity }
      }

      subgraphs[subgraphName].fields.push({ fieldName, queryFieldNode, fieldId })
    }
    ...
    for (const subgraphName of Object.keys(subgraphs)) {
      ...
      const q = collectQueries({
        ...
        fieldId,
        selection: d.fields.map(f => f.fieldName),
        types,
        fields,
        context,
        // args?
   ...
   */
  async runQueries(queries, order) {
    const result = {}
    ...
    for (const p of order) {
      ...
      this.logger.debug({ subgraph: q.subgraphName, path, query }, 'run subgraph query')
      ...
      // console.log(' --------- parentResult.data')
      // console.dir(parentResult.data, { depth: 999 })

      // console.log(' --------- q.result')
      // console.dir(q.result, { depth: 999 })
