...
function buildQuerySelection (selection, parent, wrap = true) {
  ...
  for (let i = 0; i < selection.length; i++) {
    if (selection[i].field) {
      ...
      fields.add(toQuerySelection(k.pkey || k.fkey || k.many))
  ...
  // console.dir({ selection: qselection }, { depth: 9 })
...
function keyId (k) {
  ...
  if (k.parent) {
    if (k.parent.fkey) { return `#fkey.${k.parent.fkey.field}` }
    if (k.parent.many) { return `#many.${k.parent.many.fkey}` }
  }
}
...
function deferredKeys (keys, typeName, fieldName) {
  return keys.map(k => {
    if (k.parent) {
      ...
      if (k.parent.many) {
        console.log(' ************** deferredKeys TODO parent many')
        return ''
      }
