...
function buildQuerySelection (selection) {
  ...
  for (let i = 0; i < selection.length; i++) {
    if (selection[i].field) {
      ...
      // TODO keys?
      fields.push(buildQuerySelection(selection[i].selection).selection)
    } else if (selection[i].nested) {
      fields.push(selection[i].parentField)
      for (const nested of selection[i].nested.values()) {
        // TODO keys?
        fields.push(buildQuerySelection(nested.query.selection).selection)
      }
      ...
      for (const deferred of selection[i].deferreds.values()) {
        ...
        fields.push(...deferred.keys.map(k => toQuerySelection(k.pkey)))
