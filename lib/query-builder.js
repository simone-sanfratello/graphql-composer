...
function buildQuerySelection (selection, parent, wrap = true) {
  ...
  for (let i = 0; i < selection.length; i++) {
    if (selection[i].field) {
      ...
      // TODO better code
      fields.add(toQuerySelection(selection[i].key.pkey || selection[i].key.fkey || selection[i].key.many))
      keys.set('key' + keyId(selection[i].key), selection[i].key)
...
function keyId (k) {
  ...
  if (k.many) { return `#many.${k.many.fkey}` }
}

function deferredKeys (keys, typeName, fieldName) {
  return keys.map(k => {
    if (k.fkey) {
      let key = typeName === k.entity ? k.fkey.pkey : k.fkey.field

      // support for pkey format: field.name, example author.id
      // this is not 100% accurate but as for now it's good enough
      if (!key && k.fkey.pkey.indexOf(fieldName) !== -1) {
        key = k.fkey.pkey
      }
