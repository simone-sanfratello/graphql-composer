...
function buildQuery (query, parentResult) {
  ...
  return {
    query: `${query.operation} { ${query.resolver.name}${buildQueryArgs(query.args, true)} ${selection} }`,
...
function buildSubQuery (query, parent) {
  ...
  return {
    subquery: `${buildQueryArgs(query.args, true)} ${s.selection}`,
    keys: s.keys
  }
}
...
// inputArgs are from collectArgs
function buildQueryArgs (inputArgs, root = true) {
  // TODO!!! if from query or from composer
  ...
  for (const inputArg of inputArgs) {
    args.push(valueToArgumentString(inputArg, true))
  }
...
function valueToArgumentString (inputArg, root) {
  const name = root && inputArg.name ? `${inputArg.name}:` : ''

  if (inputArg.type === 'ListValue') {
    const args = []
    for (const arg of inputArg.value) {
      const a = valueToArgumentString(arg)
      if (a === '') { continue }
      args.push(a)
    }
  ...
  if (inputArg.type === 'ObjectValue') {
    const args = []
    for (const arg of inputArg.value) {
      const value = valueToArgumentString(arg)
      if (value === '') { continue }
  ...
  const value = inputArg.type === 'StringValue' ? `"${inputArg.value}"` : inputArg.value.toString()
  return root ? name + value : value
}
