...
function buildQueryArgs (inputArgs, root = true) {
  if (inputArgs === undefined || inputArgs === null) { return '' }

  if (inputArgs.type === 'ListValue') {
    const args = []
    for (let i = 0; i < inputArgs.value.length; i++) {
      const arg = buildQueryArgs(inputArgs[i], false)
      if (arg === '') { continue }
      args.push(arg)
    }
    return `[${args.join(', ')}]`
  ...
  if (inputArgs.type === 'ObjectValue') {
    const keys = Object.keys(inputArgs.value)
    const args = []
    for (let i = 0; i < keys.length; i++) {
      const key = keys[i]
      const value = buildQueryArgs(inputArgs[key].value, false)
      if (value === '') { continue }
      args.push(`${key}: ${value}`)
    }
  ...
  // TODO test: quotes
  return inputArgs.type === 'string' ? `"${inputArgs.value}"` : inputArgs.value.toString()
  // return typeof inputArgs === 'string' ? `"${inputArgs}"` : inputArgs.toString()
}
