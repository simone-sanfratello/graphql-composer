        ...
        for (const i of Object.keys(deferred.keys)) {
          ...
          if (k.parent) {
            keys.set(p, {
              fkey: k.parent.fkey,
              many: k.parent.many,
              entity: k.entity, // TODO rename to typeName
              parent: k.parent,
...
function deferredKeys (keys, typeName, fieldName) {
  return keys.map(k => {
    if (k.parent) {
      ...
      if (k.parent.many) {
        // TODO?
        // if ((k.parent.many.as && typeName === k.parent.typeName)) {
        //   return toQuerySelection(k.parent.many.fkey)
        // // } else if (!k.parent.entity && typeName === k.parent.typeName) {
        // //   return toQuerySelection(k.parent.many.fkey)
        // }
        return ''
