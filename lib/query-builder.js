...
function buildQuerySelection (selection) {
  ...
  for (let i = 0; i < selection.length; i++) {
    if (selection[i].field) {
      ...
      // TODO keys?
      fields.add(buildQuerySelection(selection[i].selection).selection)
    } else if (selection[i].nested) {
      fields.add(selection[i].parentField)
      for (const nested of selection[i].nested.values()) {
        // TODO keys?
        fields.add(buildQuerySelection(nested.query.selection).selection)
      }
      ...
      // TODO buildQueryArgs
      fields.add('***TODO buildQuerySelection args***')
    }
  }

  return { selection: `{ ${Array.from(fields).join(' ')} }`, keys }
}
