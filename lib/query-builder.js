 ...
 */
function createDeferredQuery ({ parent, path }) {
  return {
...
function addDeferredQueryField (query, fieldName, queryFieldNode, nestedQueries) {
  query.fields.push({ fieldName, queryFieldNode })
  if (!nestedQueries) {
    query.selection.push({ field: fieldName })
    return
  }

  for (const n of nestedQueries) {
    query.selection.push({ selection: n.query.selection, wrap: false })
  }
}
  ...
  for (let i = 0; i < selection.length; i++) {
    if (selection[i].field) {
      ...
      for (const deferred of selection[i].deferreds.values()) {
        ...
        fields.add(...deferredKeys(deferred.keys))
...
function deferredKeys (keys) {
  return keys.map(k => {
    if (k.fkey) {
      return toQuerySelection(k.fkey.field ?? k.fkey.pkey)
    }
    // TODO many
