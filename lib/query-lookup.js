...
function collectQueries ({
  ...
  for (let i = 0; i < queryFieldSelections.length; ++i) {
    ...
    if (deferred) {
      const fieldPath = pathJoin(field.name, selectionFieldName)
      if (context.deferred.includes(fieldPath)) { continue }
      context.deferred.push(fieldPath)
...
function addDeferredQuery ({ deferreds, context, path, queryNode, querySelection, field, selectionFieldName }) {
  const queryPath = pathJoin(path, queryNode.queryFieldNode.name?.value)
