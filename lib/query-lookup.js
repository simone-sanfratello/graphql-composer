...
function collectQueries ({
  ...
  // const cpath = deferred ? pathJoin(path) : pathJoin(path, queryFieldNode.name?.value)
  ...
  if (rootScalar) {
    queries.set(cpath+'#'+subgraphName, queryNode)
    return { queries, deferreds }
  }
  ...
  for (let i = 0; i < queryFieldSelections.length; ++i) {
    ...
    const fieldPath = pathJoin(cpath, selectionFieldName)
    console.log('adding ', fieldPath)

    if (deferred) {
      if (context.done.includes(fieldPath)) {
        console.log('NO, deferred', fieldPath)
        continue
      }
  
      addDeferredQuery({ deferreds, context, subgraphName, path, queryNode, querySelection, field, selectionFieldName, types, fields })
      context.done.push(fieldPath)
      continue
    }
  ...
  if (queryNode.query.selection.length > 0) {
    // HERE FIX ADDING SAME QUERY MANY TIMES
    console.log('add query', cpath+'#'+subgraphName)
