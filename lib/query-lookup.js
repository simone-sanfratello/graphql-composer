...
// TODO memoize?
function collectQueries ({ queryFieldNode, field, types, info, args, subgraphName, path = '' }) {
  ...
  const fieldType = types[field.info.typeName]
  ...
  for (let i = 0; i < querySelections.length; ++i) {
    ...
    if (querySelection.selectionSet) {
      ...
      // TODO fix nested
      // queryNode.query.selection.push({ queries: collectQueries({ queryFieldNode: querySelection, fieldSrc: selectionField.src, types, info, subgraphName, path: cpath }) })
    }
  }
