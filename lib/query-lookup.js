...
function collectQueries({
  ...
  for (let i = 0; i < queryFieldSelections.length; ++i) {
    ...
    if (nested) {
      console.log(' !!! NESTED', selectionFieldName)
...
function addDeferredQuery({ deferreds, context, path, queryNode, querySelection, field, selectionFieldName }) {
  ...
  if (!deferred) {
    const xparent = queryNode.parent?.field ?? field
    console.log('WRITE', deferredParentPath, xparent.typeName)
...
function collectDeferredQueries(queryNode, nested, deferreds) {
  ...
  queryNode.query.selection.push({ deferreds: nested.deferreds, type: queryNode.field.typeName })
}
