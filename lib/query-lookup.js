...
function collectQueries ({
  ...
  // console.log('collect queries', fieldId)
  ...
  if (rootScalar) {
    // logger.debug('add query', fieldQueryPath)
  ...
  for (let i = 0; i < queryFieldSelections.length; ++i) {
    ...
    if (selection && !selection.includes(selectionFieldName)) {
      // console.log('filter', selectionFieldName, 'not in', selection.join())
      continue
    }
  ...
  // TODO remove >>>
  // if (root && queries.size < 1 && deferreds.size > 0) {
  //   order.add(fieldQueryPath)
  //   queries.set(fieldQueryPath, queryNode)
  // }
