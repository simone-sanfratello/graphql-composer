...
function collectQueries ({
  ...
  for (let i = 0; i < queryFieldSelections.length; ++i) {
    ...
    if (deferred) {
      const deferredPath = cpath + '>' + selectionFieldName
      logger.debug('query lookup, add deferred query to get', deferredPath)
      // since the selection can't be resolved in the current subgraph,
      // gather information to compose and merge the query later
      deferreds.set(deferredPath, createDeferredQuery({
