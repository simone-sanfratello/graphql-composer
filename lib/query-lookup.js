...
function collectQueries ({
  ...
  for (let i = 0; i < queryFieldSelections.length; ++i) {
    ...
    if (deferred) {
      addDeferredQuery({ deferreds, subgraphName, path, queryNode, querySelection, field, selectionFieldName, types, fields, info, logger })
      continue
    }
...
function addDeferredQuery ({ deferreds, subgraphName, path, queryNode, querySelection, field, selectionFieldName, types, fields, info, logger }) {
  const deferredParentPath = path + '>' + field.name
  const fieldPath = pathJoin(path, field.name)
