    ...
    if (field.type.entity.many && field.type.entity.many.length > 0) {
      for (const many of field.type.entity.many) {
        if (many.as) { // && (!subgraph || subgraph === many.subgraph)) {
          console.log(' ******* TODO many entityKeys')
  ...
  if (field.parent?.entity) {
    ...
    if (field.parent?.entity.many.length > 0) {
      for (let i = 0; i < field.parent.entity.many.length; i++) {
        ...
        console.log(' ******* TODO many entityKeys parent')
        // TODO if (field.typeName === key.type && (!subgraph || subgraph === key.subgraph)) {
        // keys.push({ parent: { many: key, typeName: field.parent.name } })
      }
    }
  }
